...
void Visuals::MaskChanger() noexcept
{
    ...
    if (config->misc.mask)
    {
         ...
         if (config->misc.mask) // its only applying to 1 mask rn, need to change that later with switch cases
          localPlayer->AddonBits() |= 0x10000;

        switch (config->misc.maskType)
        {
        case 1:
...
void Visuals::thirdperson() noexcept
{
    ...
    const bool bThirdPerson = config->visuals.thirdperson.enable && config->visuals.thirdperson.key.isActive() && localPlayer && localPlayer->isAlive() && !check_grenade;
    ...
    if (config->visuals.thirdperson.whileDead)
    {
        if (require_reset)
            localPlayer->getObserverMode() == ObsMode::Chase;
            ...
            if (strstr(pMaterial->getTextureGroupName(), "World") || strstr(pMaterial->getTextureGroupName(), "StaticProp")) {
                if (config->visuals.nightMode) {
                    ...
                    if (strstr(pMaterial->getTextureGroupName(), "StaticProp"))
                        pMaterial->colorModulate(0.19f, 0.19f, 0.19f);
                    else
                        pMaterial->colorModulate(0.09f, 0.09f, 0.09f);
                }
                else {
