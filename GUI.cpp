...
void renderLegitBotWindow() noexcept
{
    ...
    if (config->lgb.enableTriggerbot)
    {
        ...
        ImGui::Checkbox("Magnet", &config->lgb.magnet);
        ImGui::PopID();
    }
...
void renderRageBotWindow(ImDrawList* drawList) noexcept
{
   ...
   // ImGui::Combo("Hitbox priority", &config->rageBot[RcurrentCategory].prior_hit, "None\0Head\0Body\0");
    ...
    ImGui::hotkey2(skCrypt("Double-tap"), config->tickbase.doubletap, config->tickbase.fast_recharge && config->tickbase.defensive_dt);
    ImGui::PopID();
    ...
    ImGui::SliderInt(skCrypt("Min damage"), &config->rageBot[RcurrentCategory].minDamage, 0, 130, "%d");
...
void GUI::renderRageAntiAimWindow() noexcept
{
    ...
    if (ImGui::BeginPopup(""))
    {
     ...
     ImGui::PopID();
     ImGui::Combo(c_xor("Fake-lag"), &config->fakelag.mode, skCrypt("Static\0Adaptive\0Random\0Break LC\0"));
     ImGui::PushItemWidth(220.0f);
     ...
     static const char* hitboxes[]{ "Static legs in air", "Zero pitch on land", "No animations", "Walk in air"/*, "Move lean"*/ };
     static bool hitbox[ARRAYSIZE(hitboxes)] = { false, false, false, false };
     static std::string previewvalue = "";
     bool once = false;
     for (size_t i = 0; i < ARRAYSIZE(hitbox); i++)
     {
    ...
    if (ImGui::BeginChild(skCrypt("ESP")))
    {
        ...
        if (currentCategory <= 1)
        {
            ...
            if (ImGui::BeginPopup("")) {
               ...
               // ImGui::Checkbox(skCrypt("Show exploit"), &pCfg.showExploit);
                ImGui::EndPopup();
            }
            ImGui::PopID();
...
void GUI::renderVisualsWindow() noexcept
{
    ...
    ImGui::Checkbox(("Model scale"), &config->misc.bSkinnyBoy);
    if (&config->misc.bSkinnyBoy)
    { 
        ImGui::SliderInt(("Scale"), &config->misc.iSkinnyBoy, 0, 100);
    }
   ...
   // }
    ...
    if (ImGui::BeginPopup("Removals"))
    {
        ...
        ImGui::Checkbox(skCrypt("No Fog"), &config->visuals.noFog);
        ImGui::SameLine();
        ...
        ImGui::Checkbox(skCrypt("No bloom"), &config->visuals.noBloom);
        ImGui::SameLine();
    ...
    {
        ImGui::Combo(skCrypt("Mask type"), &config->misc.maskType, skCrypt("None\0Dallas\0Battle Mask\0Evil Clown\0Anaglyph\0Boar\0Bunny\0Bunny Gold\0Chains\0Devil Plastic\0Hoxton\0Pumpkin\0Samurai\0Sheep Bloody\0Sheep Gold\0Sheep Model\0Skull\0Template\0Wolf\0Doll\0"));
        ImGui::EndPopup();
    }
    ImGui::PopID();
    */
...
void GUI::renderMiscWindow() noexcept
{
    ...
    if (config->backtrack.fakeLatency)
        ImGui::SliderInt(skCrypt("Amount"), &config->backtrack.fakeLatencyAmount, 1, 900, "%d ms");
    ImGui::PopItemWidth();
    ...
    ImGui::Combo(c_xor("Hit Sound"), &config->misc.hitSound, skCrypt("None\0Medusa.uno\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Shield\0Clock\0Custom\0"));
    if (config->misc.hitSound == 9) {
        ImGui::InputText(c_xor("Hit Sound filename"), &config->misc.customHitSound);
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip(c_xor("audio file must be put in csgo/sound/ directory, also make sure it's .wav"));
    }
    ImGui::PushID(2);
    ImGui::Combo(c_xor("Kill Sound"), &config->misc.killSound, skCrypt("None\0Medusa.uno\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Shield\0Clock\0Custom\0"));
    if (config->misc.killSound == 9) {
    ...
    if (ImGui::Begin(Name, NULL, Flags))
    {
        ...
        ImGui::BeginChild(skCrypt("##Back"), ImVec2{ 754- 70, 559 }, false);
        {
            ...
            ImGui::BeginChild(skCrypt("##Main"), ImVec2{ 750 - 70, 555 }, false);
            {
                ...
                const char* text;
                const char* text1;
                text = " Medusa.uno (TEST)";
                text1 = " z";              
                if (text == " Medusa.uno (TEST)")
                {
                    ...
                    ImVec2 size = ImGui::GetWindowSize();
                    {
                        ...
                        draw->AddText(fonts.tahoma34, 24.f, ImVec2(pos.x + 26.5f + 1, pos.y + 2), Helpers::calculateColor(config->menu.accentColor, 0.5f), skCrypt(" Medusa.uno (TEST)"));
                        draw->AddText(fonts.tahoma34, 24.f, ImVec2(pos.x + 26.5f, pos.y + 1), ImColor(255.f, 255.f, 255.f, 255.f), skCrypt(" Medusa.uno (TEST)"));
                        ImGui::SameLine();
